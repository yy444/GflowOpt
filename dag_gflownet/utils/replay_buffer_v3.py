...
class ReplayBuffer:
    def __init__(self, capacity, num_variables):
        ...
        # 添加初始结构分数属性
        self._initial_scores = None
        
        # 维护高分样本索引的高效数据结构，O(1)访问
        self._high_score_indices_set = set()  # 用于O(1)检查索引是否在高分集合中
    ...
    def add(
        ...
        # 记录即将被替换的索引，如果这些索引在高分集合中，需要移除
        if self._is_full and self._initial_scores is not None:
            for idx in add_idx:
                if idx in self._high_score_indices_set:
                    self._high_score_indices_set.remove(idx)
        ...
        # 检查新添加的样本是否高于初始分数
        if self._initial_scores is not None:
            self._check_and_add_high_score_indices(add_idx, data['scores'])
    ...
    def _check_and_add_high_score_indices(self, indices, scores):
        """检查指定索引的样本分数，将高分样本添加到高分集合"""
        if self._initial_scores is None:
            return
        ...
        for i, idx in enumerate(indices):
            ...
            if isinstance(scores, np.ndarray):
                ...
                # 非数组类型，可能是列表或其他
                score = scores[i]
                if isinstance(score, (list, tuple, np.ndarray)) and len(score) > 0:
                    score = score[0]
            
            # 根据初始分数类型进行比较
            if isinstance(self._initial_scores, np.ndarray) and self._initial_scores.size > 1:
                env_idx = i % len(self._initial_scores)
                initial_score = self._initial_scores[env_idx]
                if isinstance(initial_score, (list, tuple, np.ndarray)) and len(initial_score) > 0:
                    initial_score = initial_score[0]
                if score > initial_score:
                    self._high_score_indices_set.add(idx)
            else:
                initial_score = self._initial_scores
                if isinstance(initial_score, (list, tuple, np.ndarray)) and len(initial_score) > 0:
                    initial_score = initial_score[0]
                if score > initial_score:
                    self._high_score_indices_set.add(idx)

    def set_initial_scores(self, scores):
        """设置初始结构的分数作为采样参考基准"""
        self._initial_scores = scores
        print(f"设置了初始结构分数: {scores}")
        
        # 重新检查所有样本，更新高分集合
        self._high_score_indices_set.clear()
        all_indices = np.arange(len(self))
        
        # 只有当有样本时才检查
        if len(self) > 0:
            all_scores = self._replay['scores'][:len(self)]
            self._check_and_add_high_score_indices(all_indices, all_scores)
            print(f"初始化了高分样本集，包含 {len(self._high_score_indices_set)} 个高分样本")
        else:
            print("没有样本可以初始化高分样本集")

    def sample(self, batch_size, rng=default_rng()):
        """
        随机采样独立的样本，只采样分数高于初始结构的样本
        采用高效的集合数据结构，复杂度不随样本数增加
        """
        if self._initial_scores is None or len(self) == 0:
            # 如果没有设置初始分数，使用原始采样方法
            indices = rng.choice(len(self), size=min(batch_size, len(self)), replace=False)
            return self._get_samples_from_indices(indices)
        
        # 从高分样本集合中采样
        if self._high_score_indices_set:
            # 将集合转换为列表用于随机采样
            high_score_indices = list(self._high_score_indices_set)
            num_to_sample = min(batch_size, len(high_score_indices))
            selected_indices = rng.choice(high_score_indices, size=num_to_sample, replace=False)
            return self._get_samples_from_indices(selected_indices)
        else:
            # 没有高分样本，使用所有样本
            print("警告: 没有找到分数高于初始分数的样本，使用所有样本采样")
            indices = rng.choice(len(self), size=min(batch_size, len(self)), replace=False)
            return self._get_samples_from_indices(indices)
    ...
    def save(self, filename):
        data = {
            ...
            'valid_trajectories': list(self._valid_trajectories),
            'initial_scores': self._initial_scores,
            'high_score_indices_set': list(self._high_score_indices_set)  # 保存高分索引集合
        }
    ...
    def load(cls, filename):
        with open(filename, 'rb') as f:
            ...
            # 加载初始分数（版本7及以上）
            if version >= 7 and 'initial_scores' in data:
                replay._initial_scores = data['initial_scores']
                print(f"从文件加载了初始结构分数: {replay._initial_scores}")
                
                # 加载高分索引集合
                if 'high_score_indices_set' in data:
                    high_indices = data['high_score_indices_set']
                    replay._high_score_indices_set = set(high_indices.tolist() if isinstance(high_indices, np.ndarray) else high_indices)
                    print(f"从文件加载了高分样本索引集合，包含 {len(replay._high_score_indices_set)} 个高分样本")
                else:
                    # 如果没有保存高分索引，重新计算
                    replay._high_score_indices_set = set()
                    all_indices = np.arange(len(replay))
                    all_scores = replay._replay['scores'][:len(replay)]
                    replay._check_and_add_high_score_indices(all_indices, all_scores)
                    print(f"重新计算了高分样本索引集合，包含 {len(replay._high_score_indices_set)} 个高分样本")
            else:
                replay._initial_scores = None
                replay._high_score_indices_set = set()
