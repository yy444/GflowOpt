...
def detailed_balance_loss(
    ...
    shift_value : float (default: 0.0)
        The shift value to add to delta_scores before scaling.
        
    scaling_factor : float (default: 1.0)
        The scaling factor to multiply delta_scores after shifting.
        
    temperature : float (default: 1.0)
        The temperature to divide delta_scores after shifting and scaling.
    ...
    # 获取原始delta_scores
    original_delta_scores = delta_scores[0]
    
    # 应用变换公式: [(delta_scores+shift_value)*scaling_factor]/temperature
    transformed_delta_scores = ((original_delta_scores + shift_value) * scaling_factor) / temperature
    
    # 在损失计算中使用转换后的delta_scores
    error = transformed_delta_scores + log_pB - log_pF + log_pi_t[-1] - lax.stop_gradient(log_pi_tp1[-1])

    normalized_error = error / normalization #归一化后，误差保持在合理范围，使训练更稳定。
    loss = jnp.mean(optax.huber_loss(normalized_error, delta=delta))

    logs = {
        'error': error,
        'loss': loss,
        'original_delta_scores': original_delta_scores,
        'transformed_delta_scores': transformed_delta_scores
    }
    return (loss, logs)
...
def log_policy(logits, stop, masks):
    ...
    logp_continue = (nn.log_sigmoid(-stop)
        + shifted_logits - jnp.log(jnp.sum(jnp.exp(shifted_logits), axis=-1, keepdims=True)))
...
def posterior_estimate(
    ...
    with trange(num_samples, disable=(not verbose), **kwargs) as pbar:
        while len(samples) < num_samples:
            ...
            actions, key, _, log_pi_global = gflownet.act(params, key, observations, 1., normalization)
