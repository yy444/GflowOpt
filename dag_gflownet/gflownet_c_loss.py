...
class DAGGFlowNet:
    ...
    def loss(self, params, target_params, key, samples, normalization):
        ...
        log_pi_t, log_pi_t_global = self.model.apply(params, key, samples['graph'], samples['mask'], normalization)
        log_pi_tp1, log_pi_tp1_global = self.model.apply(target_params, key, samples['next_graph'], samples['next_mask'], normalization)
    ...
    def act(self, params, key, observations, epsilon, normalization):
        ...
        log_pi, log_pi_global = self.model.apply(params, key, graphs, masks, normalization, is_training=False)
    ...
    def step(self, params, state, samples, normalization):
        ...
        (loss_value, logs), grads = jax.value_and_grad(self.loss, has_aux=True)(params.online, params.target, subkey, samples, normalization)

        # 梯度裁剪
        max_grad_norm = 1.0  # 设置梯度范数的最大值, 可以根据需要调整
