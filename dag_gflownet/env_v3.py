...
class GFlowNetDAGEnv(gym.vector.VectorEnv):
    def __init__(
        ...
        self._empty_graph_score = self.get_empty_graph_score()
    ...
    def get_empty_graph_score(self):
        """计算空图（没有边）的初始评分"""
        total_score = 0
        # 对每个节点计算其局部评分（此时没有父节点）
        for target in range(self.num_variables):
            key = (target, ())
            # 如果缓存中已经有这个评分，直接使用
            if key in self.local_scores:
                score = self.local_scores[key]
            else:
                # 否则计算评分并缓存
                local_score = self.scorer.get_local_scores(target, [], indices_after=None)
                # local_score返回(before, after)元组，我们只需要after部分
                score = local_score[1].score + local_score[1].prior
                self.local_scores[key] = score
            total_score += score
        return total_score
    ...
    def reset(self):
        ...
        self._state = {
            ...
            'score': np.full((self.num_envs,), self._empty_graph_score, dtype=np.float_),  # 使用空结构分数
    ...
    def step(self, actions):
        ...
        # Update the true underlying scores (保存真实的底层评分)
        self._state['score'] += delta_scores
        
        # 检查当前分数是否小于两倍初始结构分数，如果是则结束结构生成
        score_threshold = 10 * self._empty_graph_score
        # 根据分数阈值更新done状态
        low_score_mask = self._state['score'] < score_threshold
        # 将低分结构标记为已完成
        dones = dones | low_score_mask
        
        # 当episode结束时，重置为初始评分
        self._state['score'][dones] = self._empty_graph_score
        
        # 为低分结构重置状态（adjacency、mask等）
        if np.any(low_score_mask):
            # 重置低分结构的邻接矩阵
            self._state['adjacency'][low_score_mask] = 0
            # 重置低分结构的传递闭包
            self._closure_T[low_score_mask] = np.eye(self.num_variables, dtype=np.bool_)
            # 更新低分结构的掩码
            self._state['mask'][low_score_mask] = 1 - self._closure_T[low_score_mask]
            # 重置低分结构的边计数
            self._state['num_edges'][low_score_mask] = 0
            # 重置低分结构的顺序
            self._state['order'][low_score_mask] = -1
        
        # 创建返回给模型的观察值副本
        observation = deepcopy(self._state)
        
        # 为返回给模型的观察值检查评分，如果低于初始评分则使用初始评分
        # 这只影响返回值，不影响内部状态
        return_scores = np.copy(self._state['score'])
        return_scores = np.maximum(return_scores, self._empty_graph_score)
        observation['score'] = return_scores

        return (observation, delta_scores, dones, {})
